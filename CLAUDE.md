...
**Sound system (`sounds.py`)**: Sound effects use an enum-based system. `data/sfx.json` maps enum names to lists of audio files. The `Sfx` enum is generated from JSON keys at import time. Call `sounds.play_sfx(sounds.Sfx.MELEE_HIT)` at event sites to play a random sound from that enum's file list. A `_STAGGER_SECS` delay is applied between rapid successive sounds. A legacy log-based trigger system (`_EFFECT_DEFS` / `maybe_play_sfx()`) remains as an extensibility hook — `message_log.add_message()` still calls `maybe_play_sfx(text)` — but all built-in sounds use the enum system. To add a new sound: (1) add an entry to `data/sfx.json`, (2) call `sounds.play_sfx(sounds.Sfx.YOUR_KEY)` at the event site. For animation-synced sounds, pass `Sfx` enum members as sound cue values in `composite_animations()` layers.
...
- **Wished-for items are auto-identified**: `WishAction.perform()` adds the item's ID to `engine.identified_items` so the item is immediately shown by its true name, bypassing scroll/potion aliases.
...
- **Equipment enchantments**: Weapons and armor can have two kinds of enchantment. `enchantment` (int) adds to per-hit damage (weapons) or defense (armor) and is displayed as a `+N` prefix (e.g. `+1 Sword`). `enchantment_name` (str) grants a named power while equipped, displayed as an `of X` suffix (e.g. `Chain Mail of Trap Detection`). Both can appear together. Named enchantments are defined in `data/enchantments.json` and loaded via `enchantment_data.py`. Each entry specifies `id`, `label`, `effect`, `applies_to` (equipment type → `{min_floor, chance}`), and optional `equip_message`/`unequip_message`. Effect functions live in `_EFFECT_REGISTRY` in `components/equippable.py`. Numeric enchantments (+1/+2) are applied by `_maybe_enchant_item()` in `procgen.py` (15% chance for +1, scaling to 10% for +2 by floor 8). To add a new named enchantment: (1) add an entry to `data/enchantments.json`, (2) add effect functions to `_EFFECT_REGISTRY` in `components/equippable.py`.
- **Recording and replay system** (`recorder.py`, `debug.py`): Records gameplay as a snapshot + keystroke log for deterministic replay.
  ...
  - **File format**: Human-readable text with sections: `=== MAP ===` (ASCII snapshot), `=== RANDOM STATE ===` (base64 pickled `random` state), `=== KEYSTROKES ===` (one per line, e.g. `LSHIFT+UP`), `=== ENGINE STATE ===` (base64 lzma-compressed pickled Engine).
  - **Determinism**: Random state is restored before replay so identical RNG outcomes are produced. `active_recorder` / `playback_active` are module-level globals in `recorder.py` (not stored in Engine) to avoid polluting pickle state.
  - **UI overlay**: `recorder_module.render_overlay()` called from `engine.py` shows a red "● RECORD" or green "▶ PLAY" indicator in the upper-left corner during recording/playback.
